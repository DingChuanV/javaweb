  ...
  <div class="main-top">
    ...
    <div class="login-box">
      <form action="${pageContext.request.contextPath}/user/login" method="post">
        <label>用户名</label><input type="text" name="username" value="${input_msg.getUsername()}" />
        <label>密码</label><input type="text" name="password" value="${input_msg.getPassword()}" />

        <c:if test="${sessionScope.user==null}">
          <button type="submit">登录</button>
        </c:if>
        <%--注销按钮只有在session中有数据才会显示--%>
        <c:if test="${sessionScope.user!=null}">
          <button type="button"><a style="color: #ffffff" href="${pageContext.request.contextPath}/user/logout">注销</a></button>
        </c:if>

        <%--登录提示信息--%>
        <br/>
        <c:if test="${msg!=null}"><span style="color:#ff0000;">${msg}</span></c:if>
      </form>
    </div>
    <!--导航-->
    <div class="nav">
        ...
        <div class="main-text-box-bbg">
          <div class="article-box">
            ...
            <div class="source-bar">
              ...
              发布者：<%=publisher%> 分类：新闻信息 更新时间：<%=date%>
            ...
            <div class="comment">
              ...
              <dl>
                <dt class="comment-top">
                  <span class="fr">2021-09-07 09:30:56</span>
                  <b>3楼</b> <b>hz7431</b>
                </dt>
                <dd class="comment-body">
                  王铭我是你爸爸！！！
